...
mod tests {
    ...
    /// CLI-локальный аналог desktop KeyPair (только для тестов pq-модуля).
    #[derive(Debug, Clone)]
    struct TestX25519 {
        public_key: String,
        private_key: String,
    ...
    fn gen_x25519() -> TestX25519 {
        use rand::rngs::OsRng as _OsRng;
        use x25519_dalek::{PublicKey as TPub, StaticSecret as TSec};
        let private = TSec::random_from_rng(_OsRng);
        let public = TPub::from(&private);
        TestX25519 {
    ...
    fn full_setup() -> (TestX25519, TestX25519, PqKeyPair, PqKeyPair) {
        let alice = gen_x25519();
        let bob = gen_x25519();
    ...
    fn test_hybrid_rejects_legacy_ciphertext() {
        ...
        use x25519_dalek::{PublicKey as _Pk, StaticSecret as _Sk};
        let priv_arr: [u8; 32] = hex::decode(&alice.private_key).unwrap().try_into().unwrap();
        let peer_arr: [u8; 32] = hex::decode(&bob.public_key).unwrap().try_into().unwrap();
        let shared = _Sk::from(priv_arr).diffie_hellman(&_Pk::from(peer_arr));
        let cipher = _XC::new_from_slice(shared.as_bytes()).unwrap();
        let nonce_bytes: [u8; 24] = rand::random();
        let ct = cipher
            .encrypt(
                (&nonce_bytes).into(),
                _P {
                    msg: b"legacy".as_slice(),
                    aad: b"VAULT",
                },
            )
            .unwrap();
        ...
        let legacy = base64::engine::general_purpose::STANDARD.encode(&wire);
        let res = hybrid_decrypt_vault(
            &legacy,
            &bob.private_key,
            &alice.public_key,
