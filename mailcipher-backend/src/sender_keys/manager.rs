...
impl SenderKeyManager {
    ...
    pub fn derive_message_key(chain_key: &[u8; CHAIN_KEY_LEN], message_number: u32) -> [u8; MESSAGE_KEY_LEN] {
        let hk = Hkdf::<Sha256>::new(Some(b"mailcipher-sender-key"), chain_key);
    ...
    pub fn advance_chain_key(chain_key: &[u8; CHAIN_KEY_LEN]) -> [u8; CHAIN_KEY_LEN] {
        let hk = Hkdf::<Sha256>::new(Some(b"mailcipher-chain-ratchet"), chain_key);
    ...
    pub fn encrypt_for_recipient(
        ...
        let hk = Hkdf::<Sha256>::new(Some(b"mailcipher-key-distribution"), shared_bytes);
