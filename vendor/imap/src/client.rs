            ...
            let break_with = {
                ...
                match parse_response(line) {
                    Ok((
                        ...
                        // PATCH(vault): zoho и некоторые серверы присылают Done-ответ
                        // с тегом, не совпадающим с тегом отправленной команды.
                        // Оригинальный assert_eq! здесь паниковал, а с panic=abort
                        // это убивало всё приложение. Возвращаем ошибку парсинга —
                        // вызывающий код переподключится (reconnect_imap).
                        if tag.as_bytes() != match_tag.as_bytes() {
                            Some(Err((Status::Bye, Some(format!(
                                "IMAP tag mismatch: expected {match_tag}, got {:?}",
