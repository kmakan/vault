<template>
  <div class="call-overlay" :class="'call-' + state">
    ...
    <div class="call-controls">
      ...
      <!-- Исходящий: КРАСНАЯ пилюля отмены с обычной трубкой (phone) —
           перечёркнутая трубка не используется. -->
      ...
      <!-- Активный: ОДИН ряд — микрофон | большая красная
           «завершить» | динамик.
           ...
           -->
      <template v-else-if="state === 'active'">
        <!-- Соединение: ЗЕЛЁНЫЙ орб «принято» — визуальное
        ...
        <!-- Разговор установлен: ОДИН ряд — микрофон |
...
// Оверлей звонка (M3, feature/calls).
...
export default {
  ...
  data() {
    return {
      ...
      startX: null,
      dx: 0,
      // Флик-детект: быстрый свайп должен принимать звонок так же
  ...
  computed: {
    ...
    // Лёгкий свайп: latch на 70px вместо 110 — не нужно тянуть до
  ...
  watch: {
    state(s) {
      ...
      // ВАЖНО: decision тоже сбрасываем — иначе после первого
  ...
  methods: {
    ...
    onDragEnd() {
      // Флик: быстрый короткий свайп тоже
...
/* Фаза «Соединение…»: зелёный орб «принято» + маленькая красная
