...
- `App.vue` `notifyIncoming()`: вызывается только при тихом поллинге (silent=true,
  НЕ при первом скане после входа), пропускает исходящие и открытый чат; имя
  отправителя — из локального профиля/контакта.
- Ограничение: работает, пока приложение живо. Полностью закрытое приложение →
  будущий premium push-релей (M2, «будильник» через FCM/UnifiedPush, контент не
  проходит через релей).
