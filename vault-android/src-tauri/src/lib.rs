...
fn set_app_icon(app: tauri::AppHandle, icon_id: String) -> Result<(), String> {
    ...
    // Window icons are desktop-only (Windows/Linux/macOS); on mobile the
    // command is a no-op so the frontend's catch() path is not even needed.
    #[cfg(desktop)]
    {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Set up system tray (desktop only — no tray on Android/iOS)
            #[cfg(desktop)]
