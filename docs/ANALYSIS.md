# Vault — Анализ текущего состояния и план Pro-версии

> ⚠️ **УСТАРЕЛО (22.08.2026)**. Документ от 16.07, писался до переопределения
> направления. Модель «Pro ₽99/мес за темы/шрифты/emoji» **отклонена**:
> косметические пейволлы не создают ценности (шрифты пользователь поставит сам),
> а `@import` Google Fonts утекает IP в Google — противоречит zero-metadata.
> Актуальная монетизация (каналы, маркетплейс мини-приложений, premium-релеи,
> premium-функции, enterprise, донаты): **docs/ROADMAP-2026-08-16.md §4**.
> Темы оформления остаются бесплатными (6 тем в приложении).
